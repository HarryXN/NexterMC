...
# CONFIG: Put your Discord User ID here so you always have access
OWNER_ID = 1006250810142883851  # Replace with your actual User ID!
    ...
    if target_channel:
        if used_invite:
            desc = (
                ...
                f"🔗 **Invited by:** {used_invite.inviter.mention if used_invite.inviter else 'Unknown'}\n"
                f"📌 **Invite Code:** `{used_invite.code}` (Total Uses: `{used_invite.uses}`)"
            )
        else:
            desc = (
...
async def summarize(ctx):
    # Check if the user is you OR has the Manage Channels permission
    is_owner = ctx.author.id == OWNER_ID
    has_staff_perms = ctx.channel.permissions_for(ctx.author).manage_channels

    if not (is_owner or has_staff_perms):
